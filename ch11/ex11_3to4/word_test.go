...
func randomPalindrome(rng *rand.Rand) string {
	...
	runes := make([]rune, n)
	for i := 0; i < (n+1)/2; i++ {
		var r rune  // random rune up to '\u0999'
		// Ex11.4 - we only want valid unicode runes, as non-letters are ignored
		for r = rune(rng.Intn(0x1000)); !unicode.IsLetter(r); r = rune(rng.Intn(0x1000)) {}
		runes[i] = r
		runes[n-1-i] = r
	}
	return string(runes)
...
func randomNonPalindrome(rng *rand.Rand) string {
	var n int
	for n = rng.Intn(25); n <= 1; n = rng.Intn(25) {} // random length up to 24, but greater then 1
	runes := make([]rune, n)
	for i := 0; i < (n+1)/2; i++ {
		var r, s rune
		for r = rune(rng.Intn(0x1000)); !unicode.IsLetter(r); r = rune(rng.Intn(0x1000)) {}
		for s = rune(rng.Intn(0x1000)); !unicode.IsLetter(s) && r != s; s = rune(rng.Intn(0x1000)) {}
		runes[i] = r
		runes[n-1-i] = s
	}
	return string(runes)
